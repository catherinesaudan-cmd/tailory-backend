...
# -*- coding: utf-8 -*-
"""
GRILLES ET FORMES COMPOSITES — module serveur, version 2.11
===========================================================

VERSION 2.11 — 11 aout 2026.
Fichier livre sous le nom `grilles_v2_11.py` ; **a renommer `grilles.py`**
...
def analyser_page(page) -> dict:
    ...
    for table in trouvees:
        ...
        if lignes < MIN_LIGNES or colonnes < MIN_COLONNES:
            continue
        ...
        grille = {
            ...
            "contenu": contenu,
            "nature": "tableau" if taux >= TAUX_TEXTE_TABLEAU else "quadrillage",
            "zones": [],
            "comptage_refuse": None,
        }
...
def texte_avec_grilles(page, grilles: list[dict], blocs_sup=None) -> str:
    """
    ...
    if not grilles and not blocs_sup:
    ...
    for bloc in page.get_text("blocks"):
        ...
        if dans_grille:
            continue
        blocs.append((rect.y0, rect.x0, texte))
    ...
    blocs.sort(key=lambda b: (round(b[0] / 8), b[1]))
